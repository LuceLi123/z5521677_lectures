...
# printit(df.index, "df.index")
# printit(df.columns, "df.columns")
# ----------------------------------------------------------------------------
...
res = ser.loc[label_seq]  # ['2020-01-10', '2020-01-13']
printit(res, f"ser.loc[{label_seq}]")

# NOTE: Every label must exist
label_seq = ['2020-01-10', '2020-01-11']
# <example>
# res = ser.loc[label_seq] # --> will lead to the KeyError
# </example>
...
res = ser.loc['2020-01-10':'2020-01-13']  # slice includes endpoint since they only represents the label
...
# ------------------------------------------------------------
# ser.loc can be used in assignment statements:
# <target> = <expression>
# ------------------------------------------------------------
...
ser2.loc[new_label] = -99
ser2.sort_index(inplace=True)  # Will not create a new one, only sort
# ser2 = ser2.sort_index()  # Create a new series ser2 with sorted index
# ser2 = ser2.sort_index(inplace=True)  # Wrong, bind None to ser2
start = '2020-01-10'
end = '2020-01-13'
res = ser2.loc[start:end]
printit(res, f"ser2.loc['{start}':'{end}'] \nNOTE: after sorting")
...
# Sort based on the value, not the index
ser2.sort_values(inplace=True)
printit(ser2, f"The new ser2 is:")
...
res = ser.iloc[start:end]  # Endpoint is not included
...
res = ser.iloc[start:end]  # Raise no exception, since cannot out of position id endpoint is too large
printit(res, f"ser.iloc[{start}:{end}]")
...
# res = ser.iloc[start:end]  # --> Will be empty list
res = ser.iloc[::-1]
printit(res, f"ser.iloc[{start}:{end}]")
...
#   - a slice of labels
#
# Note: df.loc[row indexer] same as df.loc[row indexer, :]
#
# ----------------------------------------------------------------------------
...
res = df.loc[:, 'close']  # --> Get series
printit(res)
res = df.loc[:, ['close']]  # --> Get DataFrame
printit(res)
...
res1 = df.loc['2020-01-02', :]  # --> Get a series for the specific row label value
res2 = df.loc[['2020-01-02'], :]  # --> Get a DataFrame for the specific row label value
res2 = df.loc[['2020-01-02']]   # --> Same with Row n-1
...
#
# 1. df.loc[seq labels, label] --> Return the Series
...
#
# 2. df.loc[seq labels, [label] ] --> Return the DataFrame
rlabel_seq = ['2020-01-02', '2020-01-06']
clabel = 'close'
res = df.loc[rlabel_seq, [clabel]]
printit(res, f"df.loc[{rlabel_seq}, ['{clabel}'] ] is:")
...
# ----------------------------------------------------------------------------
# Exercise: Use df.loc['2020-01-02', slice] WITHOUT MODIFYING DF,
...
rend = '2020-01-15'
# res = df['close']
# res = df['2020-01-15']  # will not understand
res = df['close':'bday']
