...
dt_now = dt.datetime.now()  # now is a function use to create instances dt_now
print(dt_now)
...
# Now, using `repr(obj)`: create string
...
dt_now2 = dt.datetime(2023, 11, 2, 18, 41, 4, 294631)  # Useful to combine the data
print(dt_now2)

# Example:
dt_now = dt.datetime.now()
utils.pprint(dt_now, msg="This is dt_now (pretty=True)", pretty=True)  # repr version considers as pretty
...
dt1 = dt.datetime(2020, 11, 1)


utils.pprint(dt1, "dt1 is:\n", pretty=False)  # String version
utils.pprint(dt1.year, 'dt1.year')  # Print only the year (Year as the attribute)
...
msg = f"The operation:\n  {repr(dt1)} \n    - {repr(dt0)}\ngives:"
utils.pprint(delta, msg=msg)  # Hint
...
then `new_delta` is:
'''
utils.pprint(new_delta, msg, )  # Print the seconds timedelta,
# !!! timedelta only track days, seconds, microseconds
...
new_end = start + delta  # This is the new date
msg = f'''Given:
   start: {repr(start)}
   delta: {repr(delta)}
If
...
bday = dt.datetime(2000, 1, 1, hour=4, minute=1)
now = dt.datetime.now()
alive = now - bday  # Objects of timedelta
# seconds = alive.strftime('%S')
seconds = alive.total_seconds()
utils.pprint(seconds)

# 2. How old will you be in 1,340 days
fut = now + dt.timedelta(days=1340)
alive = fut - bday
age = alive.days/365
# age = alive.days/365.  # Can add a decimal point at the end of the code
utils.pprint(age, "age")

# ---------------------------------------------------------------------------- 
#   The `strftime` method: String Format Time
#   Used to create strings representing dates (in different format), can only format datetime, not datedelta
# ----------------------------------------------------------------------------
