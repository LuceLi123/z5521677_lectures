...
# <example>
# df = pd.read_csv(csv_cnts) # -> Exception, since Pandas expects the context manager
# Solution 1: We can convert the str above to something that behaves like a file object
#
...
# Alternative Solution 2: lec_utils.csv_to_df(str) -> DF
# <example>
...
dt_ser = pd.to_datetime(date_ser, format=fmt)  # Datetime type
...
df.loc[:, 'date2'] = pd.to_datetime(df.loc[:, 'date'])  # Create a new column date2
df.loc[:, 'date'] = pd.to_datetime(df.loc[:, 'date'])  # Add the min, second, micro-s to the date column
utils.pprint(df, "DF after change")
# utils.pprint(df.iloc[0, 0], "df.iloc[0,0]")  # Single object type is timestamp, but not datetime
...
# utils.pprint(df, "DF after setting the index")  # column object is datetimeindex
...
df_2020_11_0204  = df.loc['2020-11-02':'2020-11-04']  # Use slice
...
rets = df.loc[:, 'close'].pct_change()  # <mask> To compute the return
utils.pprint(rets)
